...
const NUMEROS_WHATSAPP = {
    adocao: "5581999903645",
    apadrinhamento: "5581999204111",
    contato: "5581999204111"
};
...
const caes = [
    ...
    },
    {
        ...
        historia: "Oi, eu sou o Frederico. Fui abandonado perto do abrigo e estava cheio de pulgas, precisando de cuidados e proteção. Felizmente minha saúde estava boa, e logo recebi tudo o que precisava para ficar limpinho, confortável e seguro. Mesmo depois do abandono, continuei sendo o mesmo cão carinhoso e amigável de sempre. Sou daquele tipo que se aproxima com muuuuuita energia e vontade de brincar. Gosto de companhia, de carinho, AMO BRINCAR (e destruir as coisas) e de estar por perto das pessoas. Mas também é importante te contar: eu sou um cachorro forte e cheio de energia. Nas brincadeiras, às vezes posso não perceber a minha própria força e acabar sendo mais intenso do que o ideal. Por isso, vou precisar de uma família que tenha paciência, que me guie com carinho e que invista no meu adestramento, para que eu possa aprender cada vez mais e me tornar um companheiro ainda melhor. Agora estou esperando uma família que enxergue o quanto posso ser um companheiro leal e cheio de amor para dar. Só falta alguém olhar para mim e dizer: “vem pra casa”.",
        imagem:"imagens/Frederico1.jpeg",
        tags: ["Castrado", "Vacinado", "Vermifugado"],
        destaque: false,
        paraAdocao: true,
        paraApadrinhamento: true
...
// ========== FUNÇÕES PARA MENSAGEM DE NENHUM RESULTADO ==========

function mostrarMensagemNenhumResultado(mostrar) {
    const mensagemDiv = document.getElementById('nenhum-resultado');
    const cardsGrid = document.getElementById('todos-caes');
    
    if (mensagemDiv && cardsGrid) {
        if (mostrar) {
...
function filtrarCaes() {
    ...
    const caesFiltrados = caes.filter(cao => {
        ...
        if (porteFiltro) {
            ...
            porteOk = porteCaoSemAcento === porteFiltroSemAcento ||
    ...
    if (todosContainer) {
        if (caesFiltrados.length === 0) {
            mostrarMensagemNenhumResultado(true);
        } else {
            mostrarMensagemNenhumResultado(false);
